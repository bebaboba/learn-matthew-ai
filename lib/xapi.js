...
const VERBS = {
  ...
  reset: { id: 'http://id.tincanapi.com/verb/reset', display: { 'en-US': 'reset' } },
};
...
const EVENTS = {
  ...
  // Unlike ai_topic, this event's raw question text is intentionally persisted
  // (via opts.description below) so Matthew can review and add real answers —
  // a deliberate, narrow exception to the "no raw text" rule the other events follow.
  ai_unanswered: { verb: 'asked', kind: 'ai-unanswered', fallback: 'Unanswered question' },
  // Marks "everything before this point is addressed" — the public unanswered
  // counter only counts ai_unanswered statements newer than the latest one of these.
  ai_unanswered_reset: { verb: 'reset', kind: 'ai-unanswered-reset', fallback: 'Unanswered questions reset' },
  return_visit: { verb: 'experienced', kind: 'return-visit', fallback: 'Return visit' },
  scroll_depth: { verb: 'experienced', kind: 'scroll-depth', fallback: '0' },
  time_on_page: { verb: 'experienced', kind: 'time-on-page', fallback: 'Time on page' },
};
...
export function buildStatement(eventType, label, sessionId, opts = {}) {
  ...
  if (opts.description) {
    const desc = String(opts.description).replace(/[\x00-\x1F\x7F]/g, '').slice(0, 500).trim();
    if (desc) definition.description = { 'en-US': desc };
  }
  const statement = {
  ...
  // Clamped, whole-second duration (e.g. time on page) — never trust the raw
  // client value, cap it to a plausible single-visit range.
  if (opts.duration != null) {
    const secs = Math.round(Number(opts.duration));
    if (Number.isFinite(secs) && secs > 0) {
      statement.result = { duration: `PT${Math.min(secs, 7200)}S` };
    }
  }
  return statement;
}
